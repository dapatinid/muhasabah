...
// Normalisasi ID pertanyaan agar urut sempurna 1 s.d 200
...
// Inisialisasi object jawaban kosong untuk seluruh 200 pertanyaan
questions.value.forEach(q => {
  answers.value[q.id] = { option: '', explanation: '' }
})

// Validasi ketat: Seluruh 200 pertanyaan wajib terisi pilihan ganda & penjelasannya
...
// Fungsi kompilasi laporan jawaban dan kirim via WhatsApp Web/App API resmi
const handleSendToWhatsApp = () => {
  const phoneNumber = '6285950540055' // Nomor tujuan verifikator utama
  
  let lines = [
    `*VERIFIKASI KETAT PENGGALANG DANA (200 POIN INTEGRITAS)*`,
    `======================================================`,
    ``
  ]
  ...
  questions.value.forEach((q, index) => {
    ...
    lines.push(`✍️ *Alasan/Penjelasan:* ${ans.explanation}`)
    lines.push(`------------------------------------------------------`)
    lines.push(``)
  })
...
<template>
  <Head title="Pendaftaran Penggalang Dana" />
    ...
    <div class="max-w-2xl mx-auto mt-10 p-6 bg-stone-900 border border-stone-800 rounded-3xl shadow-xl text-stone-100 transition-all duration-300">
      ...
      <div v-if="!isSteppingToQuiz" class="text-center max-w-md mx-auto py-4">
        ...
        <p class="text-xs text-amber-500 font-semibold tracking-wider uppercase mb-3">Sistem Screening Ketat Keuangan Publik</p>
        <p class="text-xs text-stone-400 mb-6 leading-relaxed text-justify bg-stone-950 p-4 rounded-2xl border border-stone-800">
          Mengingat besarnya amanah dan tingginya risiko hukum serta moral terkait pengelolaan dana masyarakat, platform menetapkan **200 poin kuesioner kelayakan**. Proses screening ini wajib diselesaikan untuk mencegah potensi fraud, malpraktik, dan penggelapan dana.
        </p>
        ...
        >
          Mulai Verifikasi (200 Pertanyaan)
        </button>
        ...
        <div class="border-b border-stone-800 pb-4 sticky top-0 bg-stone-900/95 backdrop-blur-md z-10 py-2 flex justify-between items-center">
          <div>
            <h2 class="text-base font-bold text-amber-500 flex items-center gap-1.5">
              <span>📋</span> Kuesioner Anti-Penggelapan Dana
            </h2>
            <p class="text-[10px] text-stone-400 mt-0.5">Seluruh 200 instrumen uji wajib diisi secara jujur tanpa pengecualian.</p>
          </div>
        ...
        <form @submit.prevent="handleSendToWhatsApp" class="space-y-6">
          ...
          >
            ...
            <div class="space-y-1.5">
              <span class="text-[11px] font-medium text-stone-500 block">Komitmen / Alasan Implementasi Lapangan:</span>
              <textarea
                v-model="answers[question.id].explanation"
                rows="2"
                placeholder="Tulis komitmen nyata atau penjelasan teknis tim Anda di sini..."
          ...
          <div class="pt-4 border-t border-stone-800 flex flex-col sm:flex-row gap-3 sticky bottom-0 bg-stone-900/95 backdrop-blur py-4 z-10">
            ...
            >
              <span>💬 Kirim 200 Pakta Integritas ke WA</span>
            </button>
          </div>
          <p v-if="!isFormValid" class="text-center text-[11px] text-stone-500">
            * Seluruh 200 pertanyaan verifikasi korupsi wajib diisi lengkap untuk mengaktifkan validasi kirim data.
          </p>
        </form>
      </div>
