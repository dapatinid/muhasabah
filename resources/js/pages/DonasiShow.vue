...
const formattedAuthors = computed(() => {
  ...
  const mainUser = props.donasi.user?.name || 'Hamba Allah';
  const relatedUsers = props.donasi.users || [];
  ...
  // Gunakan u.name atau u.user?.name tergantung struktur response dari Laravel Anda
...
const authorList = computed(() => {
  if (props.donasi.is_anonymous) return [];

  const list = [];
  
  // 1. Masukkan penulis utama
  if (props.donasi.user) {
    list.push(props.donasi.user);
  }

  // 2. Masukkan penulis tambahan dari relasi
  if (props.donasi.users && props.donasi.users.length > 0) {
    props.donasi.users.forEach(item => {
          ...
          <p class="text-[10px] font-bold uppercase tracking-widest text-stone-500 border-l-2 border-amber-500 pl-3 flex items-center gap-2">
            ...
            <span class="relative flex items-center">
              <button
                ...
                :disabled="donasi.is_anonymous"
                class="text-left font-medium text-emerald-500 hover:text-emerald-400 transition-colors focus:outline-none"
                :class="{ 'text-stone-400 hover:text-stone-400 cursor-default': donasi.is_anonymous }"
              >
                {{ formattedAuthors }}
              </button>
              ...
              >
                <div
                  v-if="showAuthorDropdown && !donasi.is_anonymous"
            ...
            >
              <div class="flex gap-4 items-center justify-between">
                ...
                <div class="flex flex-col items-end space-y-0.5 shrink-0">
                  <span class="text-sm font-bold font-mono" :class="log.mutation_type === 'tasyaruf' ? 'text-red-400' : 'text-emerald-400'">
                  ...
                  <div class="flex items-center gap-1">
                    ...
                    <ChevronDown 
                      v-if="log.mutation_type !== 'tasyaruf'" 
                      class="w-4 h-4 text-stone-600 transition-transform duration-300" 
                ...
                <div v-if="!log.image" class="p-4 rounded-2xl bg-stone-950 border border-stone-800/80 space-y-4">
                  ...
                  <div v-if="!log.image && log.status !== 'paid'" class="space-y-3">
                      ...
                      <button type="button" @click="payWithMayar(log.id)" :disabled="isGeneratingMayar[log.id]" class="w-full flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-stone-950 font-bold py-3 px-4 rounded-xl transition-all shadow-lg shadow-emerald-500/20 active:scale-95 disabled:opacity-50">
                          ...
                          <span class="text-xs font-bold shrink">{{ isGeneratingMayar[log.id] ? 'Memuat Sistem...' : 'TF & Konfirmasi Otomatis' }}</span>
                      </button>
                  </div>                  
                </div>
                </template>
