...
import { MapPin, Target, Star, Users, StarHalf, MessageSquare, ArrowRight, UserCircle2 } from 'lucide-vue-next'
import AppLayoutPublic from '@/layouts/AppLayoutPublic.vue'

// Import Dialog UI Components (Gunakan library dialog / modal komponen Anda)
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
// State untuk hover efek bintang saat mouse mendekat
...
function submitRating() {
    formRating.post(`/lingkaran/${props.lingkaran.slug}/rate`, {
        preserveScroll: true,
        onSuccess: () => {
  ...
  <AppLayoutPublic subtitle="Lingkaran" :title="lingkaran.nama" :show-back="true" back-href="/ukhuwah">
      ...
      <div class="max-w-3xl mx-auto px-5 relative -mt-16">
        ...
        <div class="grid grid-cols-2 gap-3 mb-6">
            <button @click="showAnggotaDialog = true" class="flex items-center justify-center gap-2 py-3 bg-stone-900 hover:bg-stone-800 border border-stone-800 rounded-2xl text-stone-200 text-xs font-bold transition-colors shadow-lg">
                ...
                <span>Anggota ({{ lingkaran.users?.length ?? 0 }})</span>
            </button>
        ...
        <div class="bg-stone-900 border border-stone-800 rounded-3xl p-6 sm:p-8">
          ...
          <div v-else class="text-center py-10 text-stone-500 text-sm">
            Deskripsi profil belum dilengkapi oleh pengurus lingkaran.
          </div>
        </div>
    ...
    <Dialog v-model:open="showAnggotaDialog">
        <DialogContent class="bg-stone-950 border border-stone-800 text-stone-200 max-w-md">
            <DialogHeader>
                <DialogTitle class="flex items-center gap-2"><Users class="size-5 text-emerald-500"/> Daftar Anggota</DialogTitle>
                <DialogDescription class="text-stone-400">
                    Daftar tokoh atau personil yang tergabung di dalam <strong>{{ lingkaran.nama }}</strong>.
                </DialogDescription>
            </DialogHeader>
            <div class="py-2 max-h-96 overflow-y-auto space-y-2.5 pr-1">
                <div v-if="lingkaran.users && lingkaran.users.length > 0" class="space-y-2">
                    <Link 
                        v-for="item in lingkaran.users" 
                        :key="item.id" 
                ...
                <div v-else class="text-center py-8 text-sm text-stone-500 italic border border-dashed border-stone-800 rounded-xl">
                    Belum ada anggota yang terdaftar di lingkaran ini.
                </div>
            </div>
        </DialogContent>
    </Dialog>
    ...
    <Dialog v-model:open="showRatingDialog">
        <DialogContent class="bg-stone-950 border border-stone-800 text-stone-200 max-w-lg shadow-2xl">
            <DialogHeader>
                ...
                <DialogDescription class="text-stone-400">
                    Berikan feedback nilai bintang dan tuliskan kesan Anda untuk {{ lingkaran.nama }}.
                </DialogDescription>
            </DialogHeader>
            ...
            <div class="space-y-5 py-2 max-h-[75vh] overflow-y-auto pr-1">
                ...
                <div class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-widest text-stone-400 border-b border-stone-800 pb-2">Riwayat Ulasan ({{ lingkaran.ratings?.length || 0 }})</h4>
                    
                    <div v-if="lingkaran.ratings && lingkaran.ratings.length > 0" class="space-y-2">
                        <div 
                            v-for="rating in lingkaran.ratings" 
                            :key="rating.id" 
                    ...
                    <div v-else class="text-center py-10 text-xs text-stone-600 italic">
                        Belum ada ulasan yang ditulis untuk lingkaran ini.
                    </div>
                </div>
