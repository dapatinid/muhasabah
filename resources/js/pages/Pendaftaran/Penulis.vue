...
<template>
    <Head title="Pendaftaran Penulis Kalam" />
      ...
      <div v-if="!isSteppingToQuiz" class="bg-stone-900 border border-stone-800 rounded-2xl p-6 sm:p-8 shadow-xl m-3">
        <h1 class="text-xl sm:text-2xl font-bold text-amber-500 mb-4">Uji Kelayakan Penulis Kalam</h1>
        <p class="text-stone-400 text-sm leading-relaxed mb-6">
          Bergabunglah sebagai kontributor literasi. Pastikan Anda memenuhi standar komitmen, integritas jurnalistik, dan regulasi media melalui <span class="text-stone-200 font-semibold">100 kriteria uji mandiri</span>.
        </p>
        <button 
      ...
      <form v-else class="bg-stone-900 flex flex-col overflow-hidden m-3">
        ...
        <div class="p-6 border-b border-stone-800 bg-stone-900/95 sticky top-0 backdrop-blur z-20">
          <h2 class="text-lg font-bold text-amber-500 mb-4">Uji Kelayakan Komunitas Kontributor</h2>
        ...
        <div class="p-4 border-t border-stone-800 flex flex-col sm:flex-row gap-3 sticky bottom-0 bg-stone-900/90 backdrop-blur z-10">
          <button
            type="button"
