...
const page = usePage()
const lingkarans = computed(() => page.props.lingkarans as any)
...
<template>
    <Head title="Manajemen Lingkaran" />
    ...
    <div class="space-y-6 p-6 max-w-7xl mx-auto">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
            <div>
                <h1 class="text-xl font-bold text-zinc-900 dark:text-white">Manajemen Lingkaran</h1>
                <p class="text-xs text-zinc-500 mt-1">Kelola data komunitas, yayasan, lembaga, dan kelompok jamaah.</p>
            </div>
            <Link href="/admin/lingkaran/create">
                <Button class="bg-rose-600 hover:bg-rose-700 text-white rounded-xl gap-2 font-bold h-11 shadow-lg shadow-rose-500/10">
                    <Plus class="size-4" />
                </Button>
            </Link>
        </div>
        ...
        <div class="flex items-center gap-3 bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
            <div class="relative flex-1 max-w-md">
                ...
                <input 
                    type="text"
                    v-model="search"
                    placeholder="Cari nama lingkaran atau jenis..."
                    class="w-full pl-10 pr-4 h-11 bg-zinc-50 dark:bg-zinc-800 border-none rounded-xl text-sm focus:ring-2 focus:ring-rose-500 text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400"
                />
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/50 text-xs font-bold text-zinc-500 uppercase tracking-wider">
                            <th class="px-6 py-4">Nama Lingkaran</th>
                    ...
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800 text-sm">
                        <tr v-if="lingkarans.data.length === 0">
                            <td colspan="7" class="px-6 py-10 text-center text-zinc-400 italic">
                                Belum ada data lingkaran ditemukan.
                            </td>
                        </tr>
                        <tr v-for="lingkaran in lingkarans.data" :key="lingkaran.id" class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span class="font-semibold text-zinc-900 dark:text-zinc-100">
                                        {{ lingkaran.ratings_avg_score ? Number(lingkaran.ratings_avg_score).toFixed(1) : '0.0' }}
                                    </span>
                                    <span class="text-xs text-zinc-400 dark:text-zinc-500">
                                        ({{ lingkaran.ratings_count || 0 }})
                                    </span>
                                </div>
                            </td>                            
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="shrink-0 size-8 rounded-full border border-zinc-200 dark:border-zinc-700 overflow-hidden bg-zinc-50 dark:bg-zinc-800/50 flex items-center justify-center">
                                        <img v-if="lingkaran.logo" :src="`/storage/${lingkaran.logo}`" class="w-full h-full object-cover" alt="Logo" />
                                    ...
                                    <div class="shrink-0 w-12 h-8 rounded-md border border-zinc-200 dark:border-zinc-700 overflow-hidden bg-zinc-50 dark:bg-zinc-800/50 flex items-center justify-center">
                                        <img v-if="lingkaran.sampul" :src="`/storage/${lingkaran.sampul}`" class="w-full h-full object-cover" alt="Sampul" />
                            ...
                            <td class="px-6 py-4 text-right">
                                <Link :href="`/admin/lingkaran/${lingkaran.id}/edit`">
                                    <Button variant="ghost" size="icon" class="hover:bg-rose-50 text-rose-600 dark:hover:bg-rose-950/30 rounded-xl size-9">
                                        <Pencil class="size-4" />
                                    </Button>
                                </Link>
                            </td>
            ...
            <div v-if="lingkarans.links.length > 3" class="flex justify-center gap-1 p-4 border-t border-zinc-100 dark:border-zinc-800 overflow-x-auto">
                <button
                    v-for="(link, i) in lingkarans.links"
                    :key="i"
                    @click="goToPage(link.url)"
                    v-html="link.label"
                    class="shrink-0 px-4 py-2 rounded-xl text-sm transition-all"
                    :class="[
                        link.active ? 'bg-rose-600 text-white font-bold' : 'bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50',
