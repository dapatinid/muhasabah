...
const form = useForm({
  ...
  deskripsi: props.lingkaran.deskripsi ?? '',
  logo: null,
  sampul: null,
  is_published: Boolean(props.lingkaran.is_published),
...
<template>
    <Head :title="`Edit Lingkaran - ${props.lingkaran.nama}`" />
    ...
    <div class="p-6">
        <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-xl font-bold text-zinc-900 dark:text-white">Edit Lingkaran</h1>
                    <p class="text-xs text-zinc-500 mt-0.5">Ubah rincian profil data lingkaran terpilih.</p>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm">
            ...
            <div v-if="user.level === 'Super Admin'" class="p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/50">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <select 
                            id="jenis" 
                            v-model="form.jenis"
                            class="flex h-11 w-full rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 text-zinc-900 dark:text-zinc-100"
                        >
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-[10px] text-zinc-500 mb-2">Pilih pengguna yang terhubung dengan lingkaran ini.</p>
                    ...
                    <div class="border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden bg-white dark:bg-zinc-950">
                        <div class="p-2 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 relative">
                            ...
                            <input 
                                ...
                                class="w-full pl-8 pr-3 py-1.5 text-sm bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500 text-zinc-900 dark:text-zinc-100"
                            />
                        </div>
                        ...
                        <div class="max-h-52 overflow-y-auto p-2 space-y-1">
                            ...
                            >
                                <input 
                                    ...
                                    :value="user.id" 
                                    v-model="form.users"
                                    class="size-4 rounded border-zinc-300 text-rose-600 focus:ring-rose-500 cursor-pointer"
                                />
                ...
                <div class="flex justify-end pt-4 border-t border-zinc-100 dark:border-zinc-800 gap-3">
                    ...
                    <Button 
                        type="submit" 
                        class="bg-rose-600 hover:bg-rose-700 text-white px-8 h-11 rounded-xl font-bold gap-2 shadow-lg shadow-rose-500/20"
    ...
    <Dialog v-model:open="showDeleteDialog">
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Hapus Lingkaran?</DialogTitle>
          <DialogDescription>
            Lingkaran bernama "<strong>{{ props.lingkaran.nama }}</strong>" akan dimasukkan ke tempat sampah (Soft Delete). Anda masih dapat memulihkannya nanti jika diperlukan.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter class="flex gap-2 justify-end">
          <Button variant="outline" class="rounded-xl" @click="showDeleteDialog = false">Batal</Button>
          <Button variant="destructive" class="rounded-xl" :disabled="form.processing" @click="deleteLingkaran">
