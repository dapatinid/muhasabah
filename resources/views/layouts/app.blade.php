...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" x-data="tallstackui_darkTheme()">
    <head>
        ...
        <title>{{ $title ?? (App\Models\Setting::get()->first()->website_name ?? 'Muhasabah.id')  }}</title>
                <!-- Favicon -->
    ...
    <x-layout>
        ...
        <x-slot:header >
            <x-layout.header without-mobile-button class="{{ request()->is('/') ? 'hidden' : 'fixed' }}">
                <x-slot:left>
                    <!-- Opening -->
                    @if (request()->is('/') || request()->is('dashboard') || request()->is('jurnal-amal') || request()->is('raport') || request()->is('users') || request()->is('user/profile') || request()->is('events') || request()->is('ticket'))
                ...
                <x-slot:middle>
                    <span class="text-base font-semibold text-black dark:text-white md:flex hidden cursor-pointer">{{ $title ?? 'Muhasabah.id' }}</span>
                    <button x-on:click="$dispatch('tallstackui-menu-mobile', { status : true })" class="md:hidden flex cursor-pointer">
                        <span class="text-base font-semibold text-black dark:text-white cursor-pointer">{{ $title ?? 'Muhasabah.id' }}</span>
                    </button>
                </x-slot:middle>
                <x-slot:right>
                    <x-dropdown>
