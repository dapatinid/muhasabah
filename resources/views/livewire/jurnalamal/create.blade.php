<div>
    ...
    <x-modal :title="__('Catatan Baru')" wire center x-on:close="$wire.resetForm()">
        ...
        <form id="jurnalamal-create" wire:submit="save" class="space-y-4">
            ...
            <div>
                <x-select.styled label="{{ __('Amalan') }} *"  wire:model.live="challenge" :options="$tantangan" searchable required/>
            </div>
            <div>
                <x-select.styled label="{{ __('Capaian') }} *"  wire:model="challenge_variant" :options="$variant" required wire:change="cekInputManual"/>
            </div>
