<div class="">
  ...
  <header class="{{ request()->is('/') ? 'fixed' : 'hidden' }}  top-0 left-0 right-0 z-40 bg-white/90 shadow">
    <nav class="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <img src="{{ asset('storage/files/LogoStrikefest2025.png') }}" alt="Logo StrikeFest" class="w-12 h-12">
      ...
      <div class="grid md:grid-cols-3 grid-cols-1 mt-4 gap-3">
        @foreach ($event as $ev)
            <a wire:navigate.hover href="{{ route('event', ['slug' => $ev->slug]) }}" class="p-2 block bg-transparent">
                <div class="flex-none shadow-lg rounded-md relative text-white"><div class="font-bold text-2xl text-center text-shadow-lg/30 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ">{{ Illuminate\Support\Str::upper($ev->title) }}</div><img src="{{ Illuminate\Support\Str::replace('%2F', '/',url('storage', $ev->cover)) }}" alt="" class="w-full object-cover aspect-video rounded-md"></div>                
                <div class="w-auto shrink mt-10 bg-white border border-gray-100 shadow-lg p-3 rounded-md  space-y-2">
                    ...
                    <div class="p-3 pb-10 [&>ul]:list-disc [&>ul]:ml-5 dark:text-gray-200 truncate line-clamp-6 min-h-80">
                        <p class="max-w-md ">
                            @php
                                $paragraf = Illuminate\Support\Str::replace('<blockquote>', '<blockquote class="relative border-s-4 border-green-500 dark:border-green-300 py-5 ps-4 sm:ps-6 bg-zinc-100 dark:bg-zinc-800 "><div class="relative z-10"><p class="text-gray-700 dark:text-white"><em>', $ev->body);
                                $paragraf = Illuminate\Support\Str::replace('</blockquote>', '</em></p></blockquote>', $paragraf);
                            @endphp
                            {!! $paragraf !!}
                        </p>
  ...
  <section id="panduan" class="max-w-6xl mx-auto py-16 px-4">
    ...
    <div class="[&>ul]:list-disc [&>ul]:ml-5 list-disc list-inside space-y-3 text-gray-700 max-w-3xl mx-auto">
      <p>
          @php
              $paragraf = Illuminate\Support\Str::replace('<blockquote>', '<blockquote class="relative border-s-4 border-green-500 dark:border-green-300 py-5 ps-4 sm:ps-6 bg-zinc-100 dark:bg-zinc-800 "><div class="relative z-10"><p class="text-gray-700 dark:text-white"><em>', $setting->panduan_lomba ?? null);
              $paragraf = Illuminate\Support\Str::replace('</blockquote>', '</em></p></blockquote>', $paragraf);
          @endphp
          {!! $paragraf !!}
      </p>
