        ...
        try {
            ...
            if (isset($data['event']) && $data['event'] === 'payment.received') {
                ...
                if ($mayarStatus === 'SUCCESS' || $mayarStatus === 'PAID') {
                    ...
                    // C. Ekstrak ID Payment Anda dari customerEmail
                    ...
                    // D. 🔥 KUNCI 1: Cari Transaksi UTAMA (Prioritaskan BUKAN infaq_sistem agar ID 12 yang terambil duluan)
                    $payment = Payment::where(function ($query) use ($linkId, $transactionId, $paymentIdFromEmail) {
                        ...
                        ->where('mutation_type', '!=', 'infaq_sistem') // <--- MEMAKSA AMBIL DONASI/TIKET/SPONSOR
                        ->where(function ($q) {
                    ...
                    if ($payment) {
                        ...
                        // E. Update Transaksi Utama (Contoh: ID 12 langsung jadi success)
                        $payment->update([
                            'status'         => 'success',
                        ...
                        // F. 🔥 KUNCI 2: Update SELURUH baris pasangan transaksi yang lahir bersamaan!
                        // Kata kunci 'infaq_sistem' dihapus agar semua baris yang satu paket (ID 11 & ID 12) dijamin lunas 100%!
                        Payment::where('paymentable_type', $payment->paymentable_type)
                        ...
                        Log::info("Webhook Mayar SUKSES: Transaksi Utama ID {$payment->id} (dan seluruh pasangannya) berhasil dilunaskan.", [
